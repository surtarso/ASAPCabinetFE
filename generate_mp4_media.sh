...
if [[ -f "$CONFIG_FILE" ]]; then
    ...
    DMD_VIDEO=$(get_ini_value "DmdVideo")
    BACKGLASS_VIDEO=$(get_ini_value "BackglassVideo")
    TABLE_VIDEO=$(get_ini_value "TableVideo")
else
...
usage() {
    ...
    echo ""
    echo -e "  ${BLUE}--now, -n${NC}                Run full mode (capture both missing table and backglass images)"
    echo -e "  ${BLUE}--tables-only, -t${NC}        Capture only the table window media. Optionally provide a specific table path"
    echo -e "  ${BLUE}--backglass-only, -b${NC}     Capture only the backglass window media. Optionally provide a specific table path"
    echo -e "  ${GREEN}--dry-run${NC}                Simulate --now mode, printing actions without executing"
    echo -e "  ${YELLOW}--dmd${NC}                    List tables missing dmd.mp4 videos and exit"
    echo -e "  ${RED}--force, -f${NC}              Force rebuilding media even if they already exist"
    echo -e "  ${RED}--clean, -c${NC}              Removes all MP4 videos (created by this script)"
    echo -e "\n  -h, --help${NC}               Show this help message and exit"
