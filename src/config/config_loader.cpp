                    ...
                    const std::string &section, const std::string &key, SDL_Keycode default_value)
{
    if (config.count(section) && config.at(section).count(key))
    {
        std::string keyName = config.at(section).at(key);
        SDL_Keycode sdlKey = SDL_GetKeyFromName(keyName.c_str());
        if (sdlKey != SDLK_UNKNOWN)
        {
            LOG_DEBUG("Loaded key '" << keyName << "' as " << sdlKey);
            return sdlKey;
        }
        else
        {
            LOG_DEBUG("Invalid key name '" << keyName << "', falling back to default " << SDL_GetKeyName(default_value));
            return default_value;
        }
    }
    LOG_DEBUG("Key '" << key << "' not found in section '" << section << "', using default " << SDL_GetKeyName(default_value));
    return default_value;
}
...
SDL_Color parse_color(const std::string &colorStr, const SDL_Color &defaultColor)
{
    ...
    if (i >= 3)
    { // At least R,G,B must be provided
