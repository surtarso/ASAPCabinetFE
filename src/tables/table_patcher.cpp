...
std::string normalizes_line_endings(const std::string& input) {
    std::string result;
...
std::string compute_files_sha256(const std::string& filename) {
    std::ifstream file(filename, std::ios::binary);
    if (!file.is_open()) {
        LOG_ERROR("TablePatcher: Failed to open file for hashing: " << filename);
        return "";
    }

    std::string content((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
    file.close();
    std::string normalized = normalizes_line_endings(content);

    EVP_MD_CTX* mdctx = EVP_MD_CTX_new();
    if (!mdctx) {
        LOG_ERROR("TablePatcher: Failed to create EVP_MD_CTX");
        return "";
    }

    if (EVP_DigestInit_ex(mdctx, EVP_sha256(), nullptr) != 1) {
        EVP_MD_CTX_free(mdctx);
        LOG_ERROR("TablePatcher: Failed to initialize SHA256 digest");
        return "";
    }

    if (EVP_DigestUpdate(mdctx, normalized.c_str(), normalized.size()) != 1) {
        EVP_MD_CTX_free(mdctx);
        LOG_ERROR("TablePatcher: Failed to update SHA256 digest");
        return "";
    }
    ...
    if (EVP_DigestFinal_ex(mdctx, hash, &hash_len) != 1) {
        EVP_MD_CTX_free(mdctx);
        LOG_ERROR("TablePatcher: Failed to finalize SHA256 digest");
        return "";
    }
        ...
        if (needsPatch(table, hashes)) {
            for (const auto& entry : hashes) {
                if (entry["sha256"] == table.hashFromVpx) {
                    ...
                    std::string computedHash = compute_files_sha256(savePath);
