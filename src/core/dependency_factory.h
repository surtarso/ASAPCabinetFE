...
class App;  // Forward declaration for ConfigUI
...
public:
    ...
    static std::unique_ptr<ISoundManager> createSoundManager(const std::string& exeDir, const Settings& settings);  // Fixed: ISoundManager
    static std::unique_ptr<ConfigService> createConfigService(const std::string& configPath);
    static std::unique_ptr<ScreenshotManager> createScreenshotManager(const std::string& exeDir, IConfigService* configService, ISoundManager* soundManager);
    static std::unique_ptr<InputManager> createInputManager(IConfigService* configService, ScreenshotManager* screenshotManager);
