...
void* lock(void* data, void** pixels) {
    ...
    if (SDL_LockMutex(ctx->mutex) != 0) {
        LOG_DEBUG("VideoPlayer: Lock mutex failed: " << SDL_GetError());
        return nullptr;
    }
    *pixels = ctx->pixels;
    return nullptr;
...
VideoContext* setupVideoPlayer(SDL_Renderer* renderer, const std::string& path, int width, int height) {
    ...
    if (!ctx->mutex) {
        LOG_ERROR("VideoPlayer: Failed to create mutex: " << SDL_GetError());
        delete ctx;
        return nullptr;
    }
    ...
    if (!ctx->instance) {
        LOG_ERROR("VideoPlayer: Failed to create VLC instance");
    ...
    if (!media) {
        LOG_ERROR("VideoPlayer: Failed to create VLC media for path: " << path);
    ...
    if (!ctx->player) {
        LOG_ERROR("VideoPlayer: Failed to create VLC media player");
        libvlc_release(ctx->instance);
        SDL_DestroyMutex(ctx->mutex);
        delete ctx;
        return nullptr;
    ...
    if (!ctx->texture) {
        LOG_ERROR("VideoPlayer: Failed to create video texture: " << SDL_GetError());
    ...
    if (!ctx->pixels) {
        LOG_ERROR("VideoPlayer: Failed to allocate pixel buffer");
    ...
    if (libvlc_media_player_play(ctx->player) != 0) {
        LOG_ERROR("VideoPlayer: Failed to start VLC playback");
        SDL_DestroyTexture(ctx->texture);
        libvlc_media_player_release(ctx->player);
        libvlc_release(ctx->instance);
        SDL_DestroyMutex(ctx->mutex);
...
void updateVideoTexture(VideoContext* video) {
    if (!video || !video->texture || !video->pixels || !video->mutex || !video->player) {
        LOG_ERROR("VideoPlayer: Invalid video context in updateVideoTexture");
        return;
    }
    if (!video->isPlaying) {
    ...
    if (SDL_LockMutex(video->mutex) == 0) {
        ...
        LOG_ERROR("VideoPlayer: SDL_LockMutex failed: " << SDL_GetError());
    }
}