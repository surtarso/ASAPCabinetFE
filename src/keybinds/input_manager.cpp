...
void InputManager::registerActions() {
    actionHandlers_ = {
        {"Previous Table",      [this]() { onPreviousTable(); }},
        {"Next Table",          [this]() { onNextTable(); }},
        {"Fast Previous Table", [this]() { onFastPreviousTable(); }},
        {"Fast Next Table",     [this]() { onFastNextTable(); }},
        {"Jump Next Letter",    [this]() { onJumpNextLetter(); }},
        {"Jump Previous Letter",[this]() { onJumpPreviousLetter(); }},
        {"Random Table",        [this]() { onRandomTable(); }},
        {"Launch Table",        [this]() { onLaunchTable(); }},
...
void InputManager::handleRegularEvents(const SDL_Event& event) {
    for (const auto& action : keybindProvider_->getActions()) {
        if (action == "Screenshot Key" || action == "Screenshot Quit") continue;

        bool matched = false;
        if (event.type == SDL_KEYDOWN && keybindProvider_->isAction(event.key, action)) matched = true;
        else if (event.type == SDL_JOYBUTTONDOWN && keybindProvider_->isJoystickAction(event.jbutton, action)) matched = true;
        else if (event.type == SDL_JOYHATMOTION && keybindProvider_->isJoystickHatAction(event.jhat, action)) matched = true;
        else if (event.type == SDL_JOYAXISMOTION && keybindProvider_->isJoystickAxisAction(event.jaxis, action)) matched = true;
...
void InputManager::onPreviousTable() {
...
void InputManager::onFastPreviousTable() {
...
void InputManager::onFastNextTable() {
...
void InputManager::onJumpPreviousLetter() {
...
void InputManager::onJumpNextLetter() {
...
void InputManager::onRandomTable() {
...
void InputManager::onLaunchTable() {
...
void InputManager::onScreenshotMode() {
    if (inExternalAppMode_) {
...
void InputManager::onToggleConfig() {
...
void InputManager::onToggleMetadata() {
...
void InputManager::onToggleCatalog() {
...
void InputManager::handleDoubleClick(const SDL_Event& event) {
    ...
    if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT) {
        ...
        auto it = lastClickTimes_.find(wid);
        if (it != lastClickTimes_.end() && (now - it->second) <= DOUBLE_CLICK_TIME) {
            LOG_DEBUG("Double-click detected");
            if (windowManager_ && settingsManager_) {
                int px,py,pw,ph, bx,by,bw,bh, dx,dy,dw,dh, tx,ty,tw,th;
                windowManager_->getWindowSetup(px,py,pw,ph, bx,by,bw,bh, dx,dy,dw,dh, tx,ty,tw,th);
                settingsManager_->updateWindowSetup(px,py,pw,ph, bx,by,bw,bh, dx,dy,dw,dh, tx,ty,tw,th);
                soundManager_->playUISound("screenshot_take");
            }
            lastClickTimes_.erase(it);
        } else {
            lastClickTimes_[wid] = now;
        }
    }
}
